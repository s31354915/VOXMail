...
Open `http://HOST:8080/`. On an empty data volume the sign-in page includes
the one-time **First-run setup** form. Once the first administrator exists,
that form is disabled permanently for that database and normal sign-in is
used.
...
provider-dependent. The container exposes TCP/UDP 8080 for HTTP and UDP
10000–10100 for the configured baresip RTP range.
